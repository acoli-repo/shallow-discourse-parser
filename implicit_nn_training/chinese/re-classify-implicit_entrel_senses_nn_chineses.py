from train_chinese import read_file, convert_relations, read_file_Org, read_file_noSenses, convert_relations_noSenses
...
def test(inputfile, outputfile, parses_file):
    ...
    #check the TokenList type of the inputfile and convert, if necessary
    ...
    #predict senses with network (only for Implicit relations!)
    ...
    #substitute senses in gold data 
	...
	try:
            ...
            #print("TokenList is ok for output")
            pass
        except IndexError:
        ...
        if rel["Type"] == '':
            if rel["Connective"]["TokenList"]==[]:
                if predicted_labels_names[counter] == u'EntRel':
                    ...
                    rel["Type"] = 'Implicit'
                #only label implicit relations, as Explicit are already labeled
